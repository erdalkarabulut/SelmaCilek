	...
	public class ShopifyManager : IShopifyService
	{
		...
		public async Task<dynamic> Ncch_Get_Shopify_LogAsync(Global global, DateTime tarih, bool yetkiKontrol = false, string ShopifyJson = "products", string ekstring = null)
		{
			...
			else
			{
				...
				apiClientId = global.apiClientId;
				apiClientSecret = global.apiClientSecret;
			}

			string token = await GetShopifyAccessToken(global);
			...
			string credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{apiClientId}:{token}"));
		...
		public async Task<string> GetShopifyAccessToken(Global global)
		{
			string shopName = "";
			string apiClientId = "";
			string apiClientSecret = "";
			...
			string baseUrl = $"https://selmacilek.myshopify.com/admin/oauth/access_token?grant_type=client_credentials&client_id={apiClientId}&client_secret={apiClientSecret}";
			...
			try
			{
				var request = new RestRequest(nextPageUrl ?? baseUrl, Method.Post);
				...
				var responseJson = JsonConvert.DeserializeObject<Dictionary<string, object>>(response.Content);
