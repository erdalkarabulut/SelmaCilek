...
{
    public partial class FrmTedarikTuruTanim : Base.FrmChildBase
    {
        private ISttdtrService _sinifService;
        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private BindingList<STTDTR> bindingList;
        private BindingList<STTDTR> oldBindingList;
        ...
        public FrmTedarikTuruTanim(ISttdtrService sinifService, IGnkxmlService sinifyerlesimService, IGnyetkService gnyetkService)
        {
            InitializeComponent();
        ...
        {
            var sttdtrs = _sinifService.Cch_GetList_NLog(global).Items;
            bindingList = new BindingList<STTDTR>(sttdtrs);
            oldBindingList = new BindingList<STTDTR>(bindingList.Select(item => item.ShallowCopy()).ToList());
            sTTDTRBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<STTDTR> response;
                var bindigSources = sTTDTRBindingSource.List;
                var datas = new List<STTDTR>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((STTDTR)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "STTDTR", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "STTDTR", id);
            form.ShowDialog();
        }
