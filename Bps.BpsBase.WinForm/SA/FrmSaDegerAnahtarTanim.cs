...
{
    public partial class FrmSaDegerAnahtarTanim : Base.FrmChildBase
    {
        private ISadegaService _sinifService;
        private SADEGA _sinif;
        private SADEGA _copysinif;

        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private BindingList<SADEGA> bindingList;
        private BindingList<SADEGA> oldBindingList;
        ...
        public FrmSaDegerAnahtarTanim(ISadegaService sinifService, IGnkxmlService sinifyerlesimService, IGnyetkService gnyetkService, IGnyetbService gnyetbService)
        {
            InitializeComponent();
        ...
        {
            var sadegas = _sinifService.Cch_GetList_NLog(global).Items;
            bindingList = new BindingList<SADEGA>(sadegas);
            oldBindingList = new BindingList<SADEGA>(bindingList.Select(item => item.ShallowCopy()).ToList());
            sADEGABindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<SADEGA> response;
                var bindigSources = sADEGABindingSource.List;
                var datas = new List<SADEGA>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((SADEGA)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "SADEGA", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "SADEGA", id);
            form.ShowDialog();
        }
        ...
        {
            SADEGA sADEGA = sADEGABindingSource.Current as SADEGA;
            _copysinif = sADEGA.ShallowCopy();
            _copysinif.Id = 0;
            FormIslemleri.ButonKontrol2(barManager, Convert.ToInt32(e.Item.Tag), yetkiModel);
            gridView1.OptionsBehavior.Editable = true;
            gridView1.OptionsView.NewItemRowPosition = NewItemRowPosition.Bottom;
        ...
        {
            SADEGA sADEGA = sADEGABindingSource.Current as SADEGA;
            GridHelper.ExcelTemplateforEntity(sADEGA);
        }
        protected override void barButExcelAktar_ItemClick(object sender, ItemClickEventArgs e)
        {
            List<SADEGA> datas = GridHelper.LoadExcelToListEntity<SADEGA>();
        ...
        private void gridView1_InitNewRow(object sender, InitNewRowEventArgs e)
        {
            ...
            if (_copysinif != null)
            {
                foreach (var property in typeof(SADEGA).GetProperties())
                {
