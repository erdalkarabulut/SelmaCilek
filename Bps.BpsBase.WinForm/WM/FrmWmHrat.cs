...
{
    public partial class FrmWmHrat : Base.FrmChildBase
    {
        private IWmhratService _sinifService;
        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private static IList<WMHRAT> list;
        private BindingList<WMHRAT> bindingList;
        private BindingList<WMHRAT> oldBindingList;
        ...
        public FrmWmHrat(IWmhratService sinifService, IGnkxmlService sinifyerlesimService, 
            IGnyetkService gnyetkService, IStftipService stftipService)
        {
            InitializeComponent();
        ...
        private void TemplateForm_Load(object sender, EventArgs e)
        {
            ...
            repLkedFisTip.DataSource = _stftipService.Cch_GetList_NLog(global, false).Items;
        }

        private void LoadGrid()
        {
            list = _sinifService.Cch_GetList_NLog(global).Items;
            bindingList = new BindingList<WMHRAT>(list);
            oldBindingList = new BindingList<WMHRAT>(bindingList.Select(item => item.ShallowCopy()).ToList());
            wMHRATBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<WMHRAT> response;
                var bindigSources = wMHRATBindingSource.List;
                var datas = new List<WMHRAT>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((WMHRAT)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "WMHRAT", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "WMHRAT", id);
            form.ShowDialog();
        }
