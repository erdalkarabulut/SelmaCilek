...
{
    public partial class FrmDepoTanimlama : Base.FrmChildBase
    {
        private IGndptnService _sinifService;
        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private static IList<GNDPTN> list;
        private BindingList<GNDPTN> bindingList;
        private BindingList<GNDPTN> oldBindingList;
        ...
        public FrmDepoTanimlama(IGndptnService sinifService, IGnkxmlService sinifyerlesimService, 
        ...
        private void TemplateForm_Load(object sender, EventArgs e)
        {
            ...
            var teknads = new List<string>() { "URYRKD" };
            var teknadsResponse = _gnthrkService.Ncch_GetThrkByMultiTeknad_NLog(global, teknads, false);

            LoadGrid();

            repUretimYeriKodu.DataSource = teknadsResponse.Items.Where(w => w.TEKNAD == "URYRKD").ToList();
        ...
        private void LoadGrid()
        {
            ...
            bindingList = new BindingList<GNDPTN>(list);
            oldBindingList = new BindingList<GNDPTN>(bindingList.Select(item => item.ShallowCopy()).ToList());
            gNDPTNBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<GNDPTN> response;
                var bindigSources = gNDPTNBindingSource.List;
                var datas = new List<GNDPTN>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((GNDPTN)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "GNDPTN", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "GNDPTN", id);
            form.ShowDialog();
        }
        ...
        private void gridView1_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            if (e.RowHandle >= 0)
            {
                ...
                var cellValue = (int)view.GetRowCellValue(e.RowHandle, gridView1.Columns["Id"]);
                if (a == null)
                {
        ...
        private void gridView1_ValidateRow(object sender, DevExpress.XtraGrid.Views.Base.ValidateRowEventArgs e)
        {
            ...
            if (view.IsNewItemRow(e.RowHandle))
            {
                ...
                if (a == null)
                {
                    if (view.IsNewItemRow(e.RowHandle))
                    {
                        focusedRowHandler.Add(new Grid(0, "Insert"));
                    }
                }
            }
        }
