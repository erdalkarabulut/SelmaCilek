...
{
    public partial class FrmDepoTip : Base.FrmChildBase
    {
        private IGndptpService _sinifService;
        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private static IList<GNDPTP> list;
        private BindingList<GNDPTP> bindingList;
        private BindingList<GNDPTP> oldBindingList;
        ...
        public FrmDepoTip(IGndptpService sinifService, IGnkxmlService sinifyerlesimService, IGnyetkService gnyetkService)
        {
            InitializeComponent();
        ...
        private void LoadGrid()
        {
            ...
            bindingList = new BindingList<GNDPTP>(list);
            oldBindingList = new BindingList<GNDPTP>(bindingList.Select(item => item.ShallowCopy()).ToList());
            gNDPTPBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<GNDPTP> response;
                var bindigSources = gNDPTPBindingSource.List;
                var datas = new List<GNDPTP>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((GNDPTP)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "GNDPTP", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "GNDPTP", id);
            form.ShowDialog();
        }
