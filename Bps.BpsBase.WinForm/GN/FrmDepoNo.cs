...
{
    public partial class FrmDepoNo : Base.FrmChildBase
    {
        private IGndpnoService _sinifService;
        private IGnkxmlService _sinifyerlesimService;
        private IGnthrkService _gnthrkService;
        private IGndptnService _gndptnService;
        private GNKXML _sinifyerlesim;

        private static IList<GNDPNO> list;
        private BindingList<GNDPNO> bindingList;
        private BindingList<GNDPNO> oldBindingList;
        ...
        public FrmDepoNo(IGndpnoService sinifService, IGnkxmlService sinifyerlesimService, IGnyetkService gnyetkService,
            IGnthrkService gnthrkService, IGndptnService gndptnService)
        {
            InitializeComponent();
        ...
        private void TemplateForm_Load(object sender, EventArgs e)
        {
            ...
            repLkedUretimYeri.DataSource = _gnthrkService.Cch_GetListByTeknad(global, "URYRKD", yetkiKontrol: false).Items;
            repLkedDepoNo.DataSource = _gnthrkService.Cch_GetListByTeknad(global, "DEPOKD", yetkiKontrol: false).Items;
            repLkedDepoKod.DataSource = _gndptnService.Cch_GetList_NLog(global, yetkiKontrol: false).Items;
        ...
        private void LoadGrid()
        {
            ...
            bindingList = new BindingList<GNDPNO>(list);
            oldBindingList = new BindingList<GNDPNO>(bindingList.Select(item => item.ShallowCopy()).ToList());
            gNDPNOBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<GNDPNO> response;
                var bindigSources = gNDPNOBindingSource.List;
                var datas = new List<GNDPNO>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((GNDPNO)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "GNDPNO", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "GNDPNO", id);
            form.ShowDialog();
        }
        ...
        private void repLkedDepoKod_Popup(object sender, EventArgs e)
        {
            ...
            string uretimYeri = gridView1.GetFocusedRowCellValue("URYRKD").ToString();
            if (uretimYeri != "")
            {
                ((GNDPNO)gNDPNOBindingSource.Current).URYRKD = uretimYeri;
                gNDPNOBindingSource.CurrencyManager.Refresh();

                editor.Properties.View.ActiveFilterCriteria = null;
                editor.Properties.View.ActiveFilterCriteria = DevExpress.Data.Filtering.CriteriaOperator.Parse("URYRKD == " + uretimYeri);
            }

        }

        private void repLkedUretimYeri_EditValueChanged(object sender, EventArgs e)
        {
            ((GNDPNO)gNDPNOBindingSource.Current).DPKODU = null;
            gNDPNOBindingSource.CurrencyManager.Refresh();
        }
    }
}
