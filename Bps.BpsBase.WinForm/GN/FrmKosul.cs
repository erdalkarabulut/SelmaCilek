...
{
    public partial class FrmKosul : BPS.Windows.Base.FrmChildBase
    {
        private IGnkoslService _sinifService;

        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private BindingList<GNKOSL> bindingList;
        private BindingList<GNKOSL> oldBindingList;
        ...
        public FrmKosul(IGnkoslService sinifService, IGnkxmlService sinifyerlesimService, 
        ...
        private void TemplateForm_Load(object sender, EventArgs e)
        {
            ...
            List<TipHareketListModel> projeList = teknadsResponse.Items.Where(w => w.TEKNAD == "PROJKD").OrderBy(w => w.TANIMI).ToList();
            repLkedProje.DataSource = projeList;
        ...
        {
            var kosullar = _sinifService.Cch_GetList_NLog(global).Items;
            bindingList = new BindingList<GNKOSL>(kosullar);
            oldBindingList = new BindingList<GNKOSL>(bindingList.Select(item => item.ShallowCopy()).ToList());
            gNKOSLBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<GNKOSL> response;
                var bindigSources = gNKOSLBindingSource.List;
                var datas = new List<GNKOSL>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((GNKOSL)bind);
                }
