...
{
    public partial class FrmSiparisFisTip : BPS.Windows.Base.FrmChildBase
    {
        private ISpftipService _sinifService;
        private IStkfytService _stkfytService;
        private SADEGA _sinif;

        private IGnkxmlService _sinifyerlesimService;
        private GNKXML _sinifyerlesim;

        private BindingList<SPFTIP> bindingList;
        private BindingList<SPFTIP> oldBindingList;
        ...
        public FrmSiparisFisTip(ISpftipService sinifService, IGnkxmlService sinifyerlesimService, 
            IGnyetkService gnyetkService, IGnevrkService gnevrkService, IStkfytService stkfytService)
        {
            InitializeComponent();
        ...
        private void LoadGrid()
        {
            ...
            bindingList = new BindingList<SPFTIP>(sadegas);
            oldBindingList = new BindingList<SPFTIP>(bindingList.Select(item => item.ShallowCopy()).ToList());
            sPFTIPBindingSource.DataSource = bindingList;
            var fiyatNoList = _stkfytService.Cch_GetListAktif_NLog(global, false).Items;
            var nullvalue = new STKFYT()
            {
               STHRTP =0,
               STFYNO =null,
               TANIMI = null
            ...
            fiyatNoList.Add(nullvalue);
            repLkeSTFYNO.DataSource = fiyatNoList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<SPFTIP> response;
                var bindigSources = sPFTIPBindingSource.List;
                var datas = new List<SPFTIP>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((SPFTIP)bind);
                }
