...
{
    public partial class FrmCariKarts : Base.FrmChildBase
    {
        ...
        private static CRCARI oldDataGenelVeriler;
        private static CRCARI currentDataGenelVeriler;
        ...
        CRADRS cariadres = new CRADRS();
        private bool ayniKayit;

        public FrmCariKarts(ICrcariService cariKartService, ICrytklService crytklService, ICrbankService crbankService,
            ICradrsService cradrsService, IMhhsplService mhhsplService, IGnyetkService gnyetkService,
            IGnthrkService gnthrkService, IGnlkhrService gnlkhrService, IGntipiService gntipiService,
            IGnevrkService gnevrkService)
        {
        ...
        protected override void barEkle_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            oldDataGenelVeriler = ((CRCARI)cRCARIBindingSource.Current).ShallowCopy();
            ...
            xtraTabControl2.SelectedTabPage = xTabPageGenelVeriler;
        }
        ...
        protected override void barDegistir_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            txtCariKodu.Enabled = false;
            oldDataGenelVeriler = ((CRCARI)cRCARIBindingSource.Current).ShallowCopy();
            if (oldDataGenelVeriler == null) return;

            var cariKodu = oldDataGenelVeriler.CRKODU;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                var cariTip = LkEdCariTuru.EditValue.ToString();
                var carievrak = new EvrakNoUretParamModel();
                carievrak.TabloAdi = "CRCARI";
                carievrak.TeknikAd = "CRKODU";
                carievrak.IslemTur = cariTip.ToInt32();
                var cikisEvrakResponse = _gnevrkService.Ncch_EvrakNoUret_NLog(global, carievrak);
                crcari.CRKODU = txtCariAnaKodu.Text = txtCariKodu.Text = cikisEvrakResponse.Nesne;
                this.Validate();
        ...
        {
            cariadres = cRADRSBindingSource.Current as CRADRS;

            var cntrlAdres = new UsrCntCariAdres(cariadres, global);
            cntrlAdres._gntipiService = _gntipiService;
            cntrlAdres._gnlkhrService = _gnlkhrService;
            grpCtrlAdresForm.Controls.Clear();
            grpCtrlAdresForm.Controls.Add(cntrlAdres);
            cntrlAdres.Dock = DockStyle.Fill;
        }

        private void xtraTabControl2_SelectedPageChanged(object sender, TabPageChangedEventArgs e)
        {
            var _tabpage = sender as XtraTabControl;
            if (_tabpage.SelectedTabPage == xTabPageGenelVeriler)
            {
        ...
        private void xtraTabControl2_SelectedPageChanging(object sender, TabPageChangingEventArgs e)
        {
            ...
            if (_tabpage.SelectedTabPage == xTabPageGenelVeriler)
            {
                currentDataGenelVeriler = ((CRCARI)cRCARIBindingSource.Current);
                var changes = CloneHelper.FindDifferences(oldDataGenelVeriler, currentDataGenelVeriler);
                if (changes.Count > 0)
                {
                    DialogResult Secim;
                    Secim = MessageBox.Show("Genel Verileri Kaydetmek istiyormusunuz ?", "Uyarı", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                    if (Secim == DialogResult.Yes)
                    {
                        barKaydet.PerformClick();
                    }
                    ...
                    {
                        CloneHelper.RevertChanges(currentDataGenelVeriler, changes);
                        cRCARIBindingSource.ResetBindings(false);
                        e.Cancel = true;
                    }
