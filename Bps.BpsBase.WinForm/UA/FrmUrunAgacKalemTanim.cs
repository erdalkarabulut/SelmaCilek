...
{
    public partial class FrmUrunAgacKalemTanim : BPS.Windows.Base.FrmChildBase
    {
        private IUakltnService _sinifService;
        private IGnkxmlService _sinifyerlesimService;
        private readonly IGnyetkService _gnyetkService;

        private UAKLTN _sinif;
        private GNKXML _sinifyerlesim;
        private static IList<UAKLTN> list;
        private BindingList<UAKLTN> bindingList;
        private BindingList<UAKLTN> oldBindingList;

        List<Grid> focusedRowHandler = new List<Grid>();
        private ProjeMenuListed yetkiModel;

        public FrmUrunAgacKalemTanim(IUakltnService sinifService,IGnkxmlService sinifyerlesimService, IGnyetkService gnyetkService)
        {
            InitializeComponent();
        ...
        private void FrmUrunAgacKalemTanim_Load(object sender, EventArgs e)
        {
        ...
        private void LoadGrid()
        {
            ...
            bindingList = new BindingList<UAKLTN>(sadegas);
            oldBindingList = new BindingList<UAKLTN>(bindingList.Select(item => item.ShallowCopy()).ToList());
            uAKLTNBindingSource.DataSource = bindingList;
        ...
        protected override void barKaydet_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                StandardResponse<UAKLTN> response;
                var bindigSources = uAKLTNBindingSource.List;
                var datas = new List<UAKLTN>();
                foreach (var bind in bindigSources)
                {
                    datas.Add((UAKLTN)bind);
                }
        ...
        protected override void barOrtamBelgeAkisi_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            FrmBelgeAkis form = new FrmBelgeAkis(global, "UAKLTN", id);
            form.ShowDialog();
        }
        ...
        protected override void barOrtamEk_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            int id = Convert.ToInt32(gridView1.GetFocusedRowCellValue("Id"));
            FrmFileManager form = new FrmFileManager(global, "UAKLTN", id);
            form.ShowDialog();
        }
